...
import {
  deleteEstateMember,
  fetchMyEstateMembers,
  updateEstateMember,
} from "../features/estateMember/estateMemberSlice";
import { IoMdArrowRoundBack } from "react-icons/io";
import { useNavigate } from "react-router-dom";

const MyEstateMemberApproval = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { myEstateMembers, emStatus } = useSelector(
    (state) => state.estateMembers,
  );
  // dispatch to the store
  useEffect(() => {
    if (emStatus === "idle") {
      dispatch(fetchMyEstateMembers());
    }
  }, [dispatch, emStatus]);
  ...
  return (
    <div>
      ...
      <p className="text-center font-bold my-10">
        Kindly engage the user to know the reason why they want to join this
        estate before you approve them
      </p>
...
export default MyEstateMemberApproval;
