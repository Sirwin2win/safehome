...
const PropertyDetailPage = () => {
    ...
    const {id} = useParams()
    const {currentProperty, propStatus,propError} = useSelector((state)=>state.properties)
  ...
  return (
    <div className='mx-20'>
        <div className='md:flex md:justify-center'>
            ...
            <p className='text-3xl font-bold mt-20'>{currentProperty?.type} {currentProperty?.estate}</p>
            <div className='flex justify-start'>
                <IoLocationOutline className='size-5 text-red-500'/>
                <p className='text-gray-500 ms-3 mb-5'>{currentProperty?.address}</p>
            </div>
       <img  className="md:h-150 md:w-full object-cover rounded-xl" src={`https://api.safehomeproperties.com/uploads/${currentProperty?.image}`} alt={currentProperty?.type} />
